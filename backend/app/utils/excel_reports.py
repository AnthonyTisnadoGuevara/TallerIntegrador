...
def crear_reporte_integral_excel(reporte: dict, metricas: dict) -> BytesIO:
    ...
    hojas = [
        ...
        ("Seguimiento semanal", ["Macroproceso", "Código evidencia", "Semana inicio", "Semana fin", "Responsable", "Acción realizada", "Nivel avance", "Porcentaje avance", "Descripción acción", "Resultado observado", "Dificultad", "Compromiso", "Requiere apoyo", "Tipo apoyo", "Archivo sustento", "Observación"], [[item.get("macroproceso"), item.get("codigo_evidencia"), item.get("semana_inicio"), item.get("semana_fin"), item.get("responsable"), item.get("accion_realizada"), item.get("nivel_avance"), item.get("porcentaje_avance"), item.get("descripcion_accion"), item.get("resultado_observado"), item.get("dificultad_encontrada"), item.get("compromiso_siguiente_semana"), item.get("requiere_apoyo"), item.get("tipo_apoyo_requerido"), item.get("archivo_sustento_url"), item.get("observacion")] for item in reporte.get("seguimientos_semanales", [])]),
